...
n_days = st.number_input('Количество дней для прогноза(прогноз более чем на 1 день вперед является экспериментальной функцией, результат может быть неточным, используйте на свой страх и риск):', value=1)
